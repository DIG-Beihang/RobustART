...
from prototype.prototype.optimizer import optim_entry, FP16RMSprop, FP16SGD, FusedFP16SGD
...
class ReparamSolver(BaseSolver):
    ...
    def setup_env(self):
        ...
        if self.config.model.kwargs.deploy:
            self.state['model'] = {('module.' + k): v for k, v in self.state['model'].items()}
        # others
...
#            pconfig['linear_b'] = {'weight_decay': 0.0}
    ...
    def repvgg_model_convert(self):
        ckpt_training = {k.replace('module.', ''): v for k, v in self.state['model'].items()}
            ...
            if curr_step > 0 and curr_step % self.config.saver.val_freq == 0:
                ...
                if self.dist.rank == 0:
                    if self.config.saver.save_many:
                        ckpt_train_name = f'{self.path.save_path}/ckpt_train_{curr_step}.pth.tar'
                        ckpt_deploy_name = f'{self.path.save_path}/ckpt_deploy_{curr_step}.pth.tar'
                    else:
                    ...
                    self.deploy_state['model'] = self.repvgg_model_convert().state_dict()
