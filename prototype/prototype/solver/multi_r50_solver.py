...
from prototype.prototype.utils.misc import makedir, create_logger, get_logger, count_params, count_flops, \
    param_group_all, AverageMeter, accuracy, load_state_model, mixup_data, \
...
class MultiEvalSolver(BaseSolver):
    ...
    def __init__(self, config,  prefix_name):
...
def main():
    ...
    for pretrain in model_list:
        ...
        tmp_config['saver']['pretrain'] = {}
        tmp_config['saver']['pretrain']["path"] = model_list[pretrain]

        solver = MultiEvalSolver(tmp_config, f"{pretrain}")
        # evaluate or train
        solver.evaluate()
