...
for ((i=0;i<5;i++)) do
    python -u -m prototype.prototype.solver.transfer_benchmark_eval_adv --config config.yaml --src_name ${model_names[${i}]} --src_path ${model_paths[${i}]} --tgt_name ${model_names[${j}]} --tgt_path ${model_paths[${j}]} --attack fgsm --eps 8/255
done
