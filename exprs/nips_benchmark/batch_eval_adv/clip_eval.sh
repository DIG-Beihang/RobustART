...
# corresponding paths
model_paths=(
    '/mnt/afs_1/huangyushi/RobustART/models/weights_convnext_base.pt'
    ...
    '/mnt/afs_1/huangyushi/RobustART/models/weights_vit_b_50_ep.pt'
...
for ((j=0;j<${#attack_names[@]};j++)) do
    python -u -m prototype.prototype.solver.clip_benchmark_eval_adv --config clip_config.yaml --src_name ${model_names[${i}]} --src_path ${model_paths[${i}]} --tgt_name ${model_names[${i}]} --tgt_path ${model_paths[${i}]} --attack ${attack_names[${j}]} --eps ${eps[${j}]}
done